...
class Formo_Core {
	...
	public function __call($function, $values)
	{
		...
		elseif (substr($function, 0, 4) == 'add_')
		{
			...
			$el = call_user_func_array(array($driver, 'shortcut'), $values);
	...
	/**
	 * open method. Format form open tag with the __formo input element
	 * $only_open_tag flag is used because when the form is rendered
	 * it will include the __formo element anyway.
	 *
	 * @return string
	 */	
	public function open($only_open_tag = FALSE)
	{
		$search = array('/{action}/','/{method}/','/{class}/','/{name}/');
		$replace = array($this->_action,$this->_method,$this->_class,$this->_formo_name);
		$open_tag = preg_replace($search,$replace,$this->_open);
		
		return ($only_open_tag)
		       ? $open_tag
		       : $open_tag."\n".$this->__formo->render();
	}
	
	/**
	 * close method. Returns the closing tag for the form
	 *
	 * @return string
	 */ 
